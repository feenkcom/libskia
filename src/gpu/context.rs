...
#[no_mangle]
pub fn skia_context_get_max_surface_sample_count_for_color_type(_ptr: *mut ValueBox<Context>, color_type: ColorType) -> usize {
    _ptr.with_not_null_return(0, |context| context.max_surface_sample_count_for_color_type(color_type))
}

#[no_mangle]
pub fn skia_context_is_color_type_supported_as_image(_ptr: *mut ValueBox<Context>, color_type: ColorType) -> bool {
    _ptr.with_not_null_return(false, |context| context.color_type_supported_as_image(color_type))
}

#[no_mangle]
pub fn skia_context_is_color_type_supported_as_surface(_ptr: *mut ValueBox<Context>, color_type: ColorType) -> bool {
    _ptr.with_not_null_return(false, |context| context.color_type_supported_as_surface(color_type))
}

#[no_mangle]
pub fn skia_context_flush(_ptr: *mut ValueBox<Context>) {
    _ptr.with_not_null( |context| { context.flush(); } );
}
