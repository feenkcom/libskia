...
pub fn skia_paint_set_shader(_paint_ptr: *mut ValueBox<Paint>, _shader_ptr: *mut ValueBox<Shader>) {
    _paint_ptr.with(|paint| match _shader_ptr.as_option() {
        ...
        Some(mut _ptr) => _ptr.with_value_consumed(|shader| {
...
pub fn skia_paint_set_image_filter(
    ...
    _image_filter_ptr: *mut ValueBox<ImageFilter>,
) {
    _paint_ptr.with(|paint| match _image_filter_ptr.as_option() {
        None => {
            paint.set_image_filter(None);
        }
        Some(mut _ptr) => _ptr.with_value_consumed(|image_filter| {
            paint.set_image_filter(Some(image_filter));
        }),
    });
}
