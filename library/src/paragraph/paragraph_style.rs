...
pub fn skia_paragraph_style_get_height(paragraph_ptr: *mut ValueBox<ParagraphStyle>) -> scalar {
    paragraph_ptr.with_not_null_return(0.0, |style| style.height())
}
...
pub fn skia_paragraph_style_set_max_lines(
    ...
    paragraph_ptr.with_not_null(|style| {
        style.set_max_lines(if max_lines == usize::max_value() {
            None
        } else {
            Some(max_lines)
        });
...
pub fn skia_paragraph_style_get_max_lines(paragraph_ptr: *mut ValueBox<ParagraphStyle>) -> usize {
    paragraph_ptr.with_not_null_return(0, |style| style.max_lines().unwrap_or(usize::max_value()))
}
